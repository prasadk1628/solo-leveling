...
// Build a unified quest object from a userQuest entry
const resolveQuest = (uq, allQuests) => {
  if (uq.is_dynamic) {
    return {
      ...
      difficulty: uq.difficulty || 'medium',
      type: uq.type || 'daily',
      xp_reward: uq.xp_reward || 25,
      is_penalty: false,
      is_dynamic: true,
      dynamic_type: uq.dynamic_type,
    };
  }
  ...
  return (
              ...
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                <div style={{ flex:1 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:4, flexWrap:'wrap' }}>
                    ...
                    {quest.is_dynamic && quest.dynamic_type === 'subject' && (
                      <span style={{ background:'var(--purple)22', color:'var(--purple)', fontSize:9, padding:'2px 6px', borderRadius:4, fontWeight:700 }}>STUDY</span>
                    )}
                    {quest.is_dynamic && quest.dynamic_type === 'project' && (
                      <span style={{ background:'var(--blue)22', color:'var(--blue)', fontSize:9, padding:'2px 6px', borderRadius:4, fontWeight:700 }}>PROJECT</span>
                    )}
                ...
                <div style={{ background:'var(--surface)', borderRadius:8, padding:'10px 12px', marginBottom:12 }}>
                  <span style={{ color:'var(--text2)', fontSize:12 }}>
                    {quest.is_dynamic && quest.dynamic_type === 'subject' ? 'Focus on this subject for 1 hour. Use active recall and practice problems for best results.' :
                     quest.is_dynamic && quest.dynamic_type === 'project' ? 'Dedicate 1 focused hour to your project. Write code, plan features, or fix bugs. Small steps daily lead to completion.' :
                     typeDesc[quest.type]}
                  </span>
                </div>
    ...
    {pomodoroQuest && (
      <Pomodoro
        ...
        onComplete={(sessions) => { handleComplete(pomodoroQuest.id); }}
